...
class Config:
    def __init__(self):
        ...
        self.initial_bandwidth = 200 * 10**3  # nr of packets/second in first month if type_bw_growth is not BW_ZERO
        ...
        self.emission_rate = 0.02  # emit 2% of inflation pool each month
        ...
        # percentage of bandwidth fees allocated to the set of mix nodes and the set of gateways
        ...
        self.deposit_gw = 100  # in token, minimum locked deposit required to register as gateway
        ...
        # bw pricing parameter: key variable! (key question: what pricing per packet makes the system viable long-term?)
        self.price_packet_initial_dollar = 1 / 10**6  # $ per packet: price per packet paid by end users buying Nym bw

        # cost_packet_bw_initial_dollar is a key parameter! determines operational cost for both mixes/gateways
        # value is zero when nodes have flat operational rates with unlimited bandwidth
        ...
        # monthly flat costs of mixes and validators (gateways are assumed to cost the same as mixes)
        ...
        # network size parameters, relevant to functions in Network_econ.py for setting the number of mixes and gateways
        ...
        self.nr_min_mixes = 120 #720  # k is never smaller than this number, regardless of (no) user traffic
        self.min_mixnet_width = 20 #120  # minimum network width: determines the number of active mixes (LW)
        ...
        # a value of 5 implies 5x peaks can be handled and average load: 1/5 = 20% -- and 80% wasted (idle) cpu capacity
